class methods {
    ...
    Verifytext1indexbyxpath(xpath, expectedText, int) {
        cy.xpath(xpath, { timeout: 60000 }).eq(int).should('include.text', expectedText);
    }

    Verifytext2indexelement(selector, expectedText, int) {
        cy.get(selector, { timeout: 60000 }).eq(int).should('include.text', expectedText);
    }
